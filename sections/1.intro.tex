...
The introduction of the quark model explained the properties of the numerous hadrons in terms of six kinds of quarks, and the theory was given the name \emph{quantum chromodynamics} (QCD). This, together with the well-established quantum theory of electromagnetism, \emph{quantum electrodynamics} (QED), became the first approximation to the standard model \cite{Weinberg_2004-history}.
...
The dissimilarity of QCD to QED can be credited largely to the fact that the gauge group $\SU(3)$ is non-Abelian, corresponding to the fact that the gluon force carriers of QCD are themselves colour-charged, which results in asymptotic freedom \cite{Gross_2005}.
This is in contrast to the Abelian $\U(1)$ theory of QCD, in which the force carriers are uncharged photons.
Despite the technical problems which come with its complex non-perturbative nature, quantum chromodynamics remains a highly successful and beautiful theory.

% It is successful in the sense that offers precise predictions
However, QCD comes with a mystery of theoretical concern: the \emph{strong \CP\ problem}, or ``why does QCD fail to forbid \CP\ violation?''
...
Instead, the disparity indicates a deeper theoretical problem, and provides good reason to pursue physics beyond the standard model.

This chapter fills in the background relevant to the formal statement of the strong \CP\ problem, and assumes surface-level familiarity with differential geometry, Lagrangian mechanics, and quantum mechanics, and in particular exterior calculus.
...
This overview aims to provide a basic understanding of the important mathematical objects, or `moving parts', of a gauge field theory and their roles and interpretations within the context of the standard model.
The chapter later introduces further concepts specific to QCD and the strong \CP\ problem on an as-needed basis.
...
In geometrical language, the matter field $\bm ψ \in \secs(\cal V)$ is a section of a vector bundle $\fibrebundle{V}{\cal V}{\cal M}$.
...
Thus, we may write the action as matrix product,
\begin{math}
...
This new theory, with the original equations of motion, is physically identical to the original.
...
At each point in spacetime, and the gauge field $\tsbm A$ linearly assigns to each direction in spacetime an infinitesimal transformation of the matter field (this is why it is Lie algebra valued).
...
However, the covariant derivative $\ts\nabla : \secs(\cal V) \cong \forms^0(\cal M, \cal V) \to \forms^1(\cal M, \cal V)$ is not readily defined on 1-forms such as $\tsbm A$ until we make the natural extension\footnotemark{} to a \emph{covariant exterior derivative}
...
.\end{align}
\footnotetext{
	% Note that $\secs(\cal V) = \forms^0(\cal M, \cal V)$.
	...
	\end{math}
	for $\ts φ \in \forms^p(\cal M)$ and $\bm ψ \in \secs(\cal V)$, analogous to the usual exterior derivative.
}%
...
.\end{align}
% which is expressed here in various equivalent ways by using a basis $\qty{\bm e_a}$ and dual basis $\qty{\bm e^a}$ of $\cal V$ (note that $V$ is equipped with the Euclidean inner product; $\bm e^a (\bm e_b) = δ^a_b$).
...
The gauge field strength automatically satisfies the Bianchi identity, $\covdd\tsbm F = 0$.
...
Explicitly, the quantum mechanical amplitude that the fields $\bm ψ$ and $\tsbm F$ satisfy prescribed boundary conditions on $\partialΩ$ surrounding some region of spacetime $Ω \subseteq \cal M$ is given by the path integral
\begin{align}
...
An important component of a gauge theory's equations of motion are the terms in the Lagrangian which describe the dynamics of the gauge field $\tsbm A$.
...
(Since the Lie algebra of $\U(1)$ is $\RR$, the 2-form $\ts F$ is scalar-valued in QCD.)
Expressing
% \begin{math}
% 	\tsbm F
% 	= \ts\dd ct\wedge\tsbm E + \tsbm B
% 	= \bm E_i\,\ts\dd ct\wedge\ts\dd x^i + \bm B_i \star(\ts\dd ct\wedge\ts\dd x^i)
% \end{math}
\begin{math}
...
The other term $\angbr{\tsbm F \wedge \tsbm F}$ is known as the \emph{Chern--Simons term} or the \emph{topological $θ$-term}, for reasons which will become apparent after a survey of its properties.
\begin{itemize}
\item 
...
Historically, the $θ$-term was dismissed as unphysical, since at first sight it appears to be gauge-dependent.
It was only with the discovery of \emph{instantons} and \emph{the non-trivial vacuum} in the mid 1970s \cite{first_instantons_1975} that it was realised that the $θ$-term should be considered, and does not necessarily vanish \cite{instantons-whats-happening}.
...
The non-trivial vacuum is not an obvious feature, as it is absent in Abelian theories such as QED, out of which QCD emerged.
...
This means that there are topologically inequivalent gauge field configurations $\tsbm A$, with the winding number labelling each distinct topological class.
...
Small gauge transformations are not measurable and describe the same vacuum state, whereas large ones $g_n \cdot \tsbm A_0$ \emph{are}, hence describing \emph{distinct vacua} $\ket{n}$ labelled by winding number.
...
Under independent axial rotations of each of the quark fields, the Yukawa mass term is invariant \emph{if} the quark masses also shift phase.
...
Furthermore, there is a strong argument that the inclusion of the \CP-violating term is ``natural.''
