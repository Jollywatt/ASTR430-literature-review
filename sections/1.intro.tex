...
If the connection is required to transform like the matter field $\bm ψ$ under local gauge transformations, that is, as
\begin{align}
...
% All that remains in our theory to specify are the equations of motion for $\bm ψ$ and $\tsbm A$.
...
% We now have constructions for the matter field $\bm ψ$, its derivative $\ts\nabla_{\!A}\bm ψ$ and the strength of the gauge field $\tsbm F$ which all transform regularly under gauge transformations.
% All that remains to be specified in our theory are the equations of motion, which are to be expressed in terms of these three geometrical objects in a gauge invariant manner.
...
\footnotetext{
	Technically, we consider the subset of $\forms^1(\cal M, \mathfrak g)$ consisting of gauge fields $\tsbm A$ which approach zero at spatial infinity. 
}
...
The matter field $\bm ψ = \bm φ_{(1)} \oplus \cdots \oplus \bm φ_{(n)}$ may now more generally be comprised of multiple fermion fields, which may be rotated into one another by the gauge group.
...
The $θ$-term $\angbr{\tsbm F\wedge\tsbm F}$ also arises in quantum field theories of fermions via a seemingly unrelated source: the \emph{chiral anomaly}.
...
,&	\text{i.e.,}\quad
	j^{\,μ} &= \bar{\bm ψ} \bm γ^μ \bm ψ
	\quad\text{where}\quad
	\star\ts J = j_μ \, \ts\dd x^μ
,\end{align}
whose continuity equation reads $\ts\dd\ts J = 0$ (i.e., $\partial_μj^μ = 0$).
Classically, this corresponds to conservation of charge, and in the quantum regime, to the number of particles.

The Dirac Lagrangian classically exhibits another global Abelian symmetry, given by \emph{axial fermion rotations} $\U(1)_A$, which transform left- $\bm ψ_+$ and right-handed $\bm ψ_+$ fermion components differently:
\begin{align}
	\bm ψ &\overset{\U(1)_A}\mapsto e^{i\bm γ^5 α}\bm ψ
,&	\text{i.e.,} \quad
	\bm ψ_\pm &\overset{\U(1)_A}\mapsto e^{\pm iα}\bm ψ_\pm
.\end{align}
...
The continuity equation $\ts\dd\ts J = 0$ is violated by the presence of none-other than the $θ$-term,
\begin{align}
	\ts\dd\ts J_A = \angbr{\tsbm F \wedge \tsbm F}
	\label{eqn:chiral-anomaly}
.\end{align}
This leads to parity violation...
...
\begin{align}
	\cal L_\text{QCD} = \overbar{\bm ψ}\qty(i\bm γ^μ\nabla_μ - m)\bm ψ
