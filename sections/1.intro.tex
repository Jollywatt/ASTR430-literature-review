...
\\	\covdd\tsbm φ &= \ts\dd\tsbm φ + \tsbm A\wedge\tsbm φ
% \\	\qty(\partial_μ φ_ν{}^a + A_μ{}^a{}_b φ_ν{}^b) \, \ts\dd x^μ \otimes \bm e_a
.\end{align}
\footnotetext{
...
,\end{align}
where we have written the r.h.s.\ with respect to a basis $\qty{\bm e_a}$ and dual basis $\qty{\bm e^a}$ of $\cal E$ (note that $V$ is equipped with the Euclidean inner product; $\bm e^a (\bm e_b) = δ^a_b$).
The field strength $\tsbm F$ is useful because it is geometrical, in the sense that it transforms like the matter field; $\tsbm F \mapsto g\cdot\tsbm F = g_ρ\tsbm F g_ρ^{-1}$ under gauge transformations, even though $\tsbm A$ does not.
...
.\end{align}
It is in this sense that the $θ$-term is a consequence of the non-trivial vacuum in QCD.
...
\begin{align}
	\LL_\text{Dirac} = \bar{\bm ψ}\qty(i\hbar c\bm γ^μ\nabla_μ - mc^2)\bm ψ \;\vol
...
In quantum theories of fermions, the $θ$-term makes another important appearance, arising via the \emph{chiral anomaly}.
...
This means that $\ts J_A$ is not conserved---and hence \CP\ symmetry is violated---in the presence of instantons.
...
Pure QCD with a single quark is a quantum gauge theory with the matter field $\bm ψ = \bm φ \otimes \bm c$, where $\bm c \in \CC^3$, which can be viewed as a fermion with components in \emph{colour space}, $\CC^3$.
The gauge group is $\SU(3)$, and its action on $\bm ψ$ is to transform colour space under the fundamental representation.



Pure $N$-quark QCD is a quantum gauge theory whose matter field is a direct product of $N$ fermionic Dirac fields, describes $N$ types, or \emph{flavours}, of quark.
The and whose gauge group $\SU(N)$ gives rise to $N^2 - 1$ gauge fields, or \emph{gluons}.
...
\begin{align}
	\LL_\text{QCD} &= \bar{\bm ψ}\qty(i\hbar c\bm γ^μ\nabla_μ - mc^2)\bm ψ \, \vol- \angbr{\tsbm F \wedge\star \tsbm F}\hbar
...
\\	\text{i.e.,} \quad 
	\cal L_\text{QCD} &= \sum_{q=1}^N\overbar{ψ}_a^{(q)}\qty(i\hbar cγ^{μ a}{}_b\nabla_μ - mc^2\delta^a{}_b)ψ^b_{(q)} - \frac\hbar4F^a{}_{bμν}F_a{}^{bμν}
\end{align}
where
