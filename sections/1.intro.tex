...
\footnote{The partial derivative $\pdv*{\LL}{\ts\nabla\bm ψ}$ of a volume form $\LL$ with respect to a 1-form $\ts\nabla\bm ψ$ is defined by extending the scalar partial derivative to be an anti-derivation on differential forms. For details, see \cite{mitsou2013differential}.}
...
The Chern--Simons term is odd under both time-reversal symmetry $T$ and parity $P$ (notice $\epsilon^{μν\rho\sigma} \mapsto -\epsilon^{μν\rho\sigma}$ under $T$ or $P$) but not under charge conjugation $C$.
...
\begin{align}
	n = \frac{1}{8\pi^2}\int_{\cal M} \angbr{\tsbm F \wedge \tsbm F} \in \ZZ
...
% However, for non-Abelian gauge groups such as the $\SU(3)$ of QCD, the space of gauge fields $\forms^1(\cal M, \mathfrak g)$ has a non-trivial topology,\footnotemark\ containing disjointed regions labelled by $n \in \ZZ$.
% \footnotetext{
...
In this context, the Chern--Simons term is also referred to as the $θ$-term.


\subsubsection{The \texorpdfstring{$θ$-Term}{θ-Term} is a Consequence of the Non-trivial Vacuum}

The $θ$-term is more than just a mathematical possibility which lacks reason for its inclusion in  physical theories.
It is in fact central to non-Abelian gauge theories as a direct consequence of their \emph{non-trivial vacuum structure}.

For an Abelian gauge group $\U(1)$, the space of gauge transformations $\secs(\cal G)$ is continuously connected to the identity, and the winding number is zero for all $\tsbm A \in \forms^1(\cal M, \mathfrak g)$.
Consequently, the $θ$-term (with fixed $θ$) is neither relevant in classical electromagnetism nor in QED.
However, for non-Abelian gauge groups such as the $\SU(3)$ of QCD, the space of gauge transformations\footnotemark\ $\secs(\cal G)$ has a non-trivial topology, with path-disconnected regions labelled by some $ν \in \ZZ$.
(This is precisely because the third homotopy group of $\cal G$ is $π_3(\cal G) = \ZZ$. \note{Why third? Has a 3+1 split snuck in here? Is this general or specific to $\SU(3)$ and (3+1)D?})
\footnotetext{
	Specifically, we consider the space of asymptotically-identity gauge transformations $\secs^{\iden}(\cal G)$ (whose elements are maps $g : \cal M \to G$ with $g(x) \to \iden$ as $x \to \infty$), to ensure that $\tsbm A$ satisfies prescribed boundary conditions.
	\note{Who realised this? 't Hooft? Wigner?}
}
Elements in the identity-connected component of $\secs(\cal G)$ are named \emph{small} gauge transformations, and all others \emph{large}.
A large gauge transformation $g_ν \in \secs(\cal G)$ shifts the winding number $n$ of a gauge field $\tsbm A$ to $n + ν$, giving rise to $\ZZ$-many gauge-equivalent fields $\qty{g_ν \cdot \tsbm A}_ν$ which cannot be continuously gauge transformed into one another.
Because of the involvement of large gauge transformations, the physical implications of a topologically-rich $\secs(\cal G)$ are inherently non-perturbative.
Of particular relevance are the implications for the QCD vacuum.
...
Not only is this consistent with an identically vanishing gauge field $\tsbm A_0 = \tsbm 0$, but also with gauge transformations \eqref{eqn:gauge-field-transformation-law} of zero,
\begin{align}
	g \cdot \tsbm A_0 = (\ts\dd g_ρ)(g_ρ)^{-1}
.\end{align}
Large gauge transformations $g_n$ lead to `perturbatively distinct' vacua $\ket{n}$, each labelled by winding number.
...
Denoting by $\braketpm{n}{m}$ the quantum amplitude that the vacuum state $\ket{n}$ at $t \to -\infty$ evolves to $\ket{m}$ at $t \to \infty$, then the vacuum-to-vacuum amplitude is
\begin{align}
	\braketpm{θ}{θ}
...
In the path integral formulation \eqref{eqn:path-integral}, the amplitude $\braketpm{n}{n + ν}$ can also be expressed explicitly
\begin{align}
 	\braketpm{n}{n + ν} =
 	\int_{\partial\Omega}\! \cal D[\tsbm A; ν] \exp\qty{\frac{i}{\hbar} \int_{\Omega}\! \LL}
	\label{eqn:vacuum-transition-2}
,\end{align}
where $\cal D[\tsbm A; ν]$ means that the path integral is over all gauge fields with winding number $ν$.
Combining \eqref{eqn:vacuum-transition-1} and \eqref{eqn:vacuum-transition-2}, the amplitude of evolution from and to the gauge invariant vacuum is
\begin{align}
	\braketpm{θ}{θ}
	&= \sum_ν e^{iθν} \int_{\partial\Omega}\! \cal D[\tsbm A; ν] \exp\qty{\frac{i}{\hbar} \int_{\Omega}\! \LL}
\\	&= \sum_ν \int_{\partial\Omega}\! \cal D[\tsbm A; ν] \exp\qty{\frac{i}{\hbar} \int_{\Omega}\! \LL + iθν}
,\intertext{
which, using \eqref{eqn:winding-number} to write $iθν$ as an effective term in the Lagrangian, is
}
	&= \int_{\partial\Omega}\! \cal D[\tsbm A] \exp\qty\bigg{\frac{i}{\hbar} \int_{\Omega}\! \qty\bigg[\underbrace{\LL + θ\frac{\hbar}{8π^2}\angbr{\tsbm F\wedge\tsbm F}}_{\displaystyle\LL_\text{eff}}]}
.\end{align}
Thus, the non-perturbative nature of the vacuum can be encapsulated in an effective Lagrangian $\LL_\text{eff}$, which includes the $θ$-term.
...
The entire matter content of the standard model, including quarks and leptons, is comprised purely of fermion fields.
...
Proceeding with the assumption that $θ \ne 0$, one finds that the strong force now violates \CP.
A physical prediction of the standard model modified with such a \CP-violating QCD sector is that the neutron is expected to possess an electric dipole moment of approximate magnitude $|d_n| \approx 10^{-18} \,e\,\mathrm{cm}$.
In reality, current measurements \cite{electric_dipole_neutron_2020} of the neutron's electric dipole moment yield a tight upper bound of $|d_n| \lesssim 10^{-26} \,e\,\mathrm{cm}$, which in turn implies a stringent constraint on the $θ$-parameter, $|θ| \lesssim 10^{-10}$ \cite{ParticleDataGroup-review-2020}.
Thus, the $θ$-term is not presently considered to be part of the standard model.


However, the inclusion of the $θ$-term is ``natural'' because we lack reason to exclude it on a theoretical basis: it is Lorentz and gauge invariant, etc., just like its \CP-symmetric counterpart, $\angbr{\tsbm F \wedge\star\tsbm F}$; it arises via chiral anomaly; and it is a consequence of the non-trivial vacuum structure of QCD.
From an empirical perspective, if no \CP-violating interactions were observed in Nature, then $θ$ could be set to zero on the basis of symmetry.
However, the weak interaction is explicitly parity-violating.
...
The strong \CP\ problem is the question, ``why, then, is $θ$ so small?''
